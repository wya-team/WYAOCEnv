...
这篇指南会指导你如何为 `WYAiOSEnv` 贡献一份自己的力量，请在你要提 `issue` 或者 `pull request` 之前花几分钟来阅读一遍这篇指南。
...
我们使用 [`GitHub Issues`](https://github.com/wya-team/WYAiOSEnv/issues) 来做 `bug` 追踪。
...
在你 `clone` 了 `WYAiOSEnv`  的代码并且使用 `*** install` 安装完依赖后，你还可以运行下面几个常用的命令：
