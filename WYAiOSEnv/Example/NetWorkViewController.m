...
- (void)exampleUploadImage {
    ...
    [button addCallBackAction:^(UIButton * button) {
        [BaseNetWorking UploadImageWithUrl:@"https://wyaoa.ruishan666.com/uploadfile/upimg.json?action=uploadfile&encode=utf-8&code=xcx"
            params:nil
            Images:@[ image ]
            ImageNames:@[ @"navbgimg" ]
            ImageType:imageForPNG
...
- (void)exampleUploadOneFile {
    ...
    [button addCallBackAction:^(UIButton * button) {
        ...
        [BaseNetWorking UploadFileWithUrl:@"https://wyaoa.ruishan666.com/uploadfile/upimg.json?action=uploadfile&encode=utf-8&code=xcx"
            Params:nil
            FilePaths:@[ path ]
            FileNames:@[ @"git操作" ]
            progress:^(float progress) {
...
- (void)exampleDownloadOneFile {
    ...
    [button addCallBackAction:^(UIButton * button) {
        [BaseNetWorking DownloadFileWithUrl:@""
            Params:nil
            FilePath:string
            progress:^(float progress) {
                NSLog(@"progress==%f", progress);
